...
class BattleShip:
	def __init__(self, rows_grid, columns_grid, co_ord_symbol):
		...
		# Placing ship randomly on board x refers to array, y refers to index in array
		self.ship_x = randint(0, rows_grid)
		self.ship_y = randint(0, columns_grid)

	def create_board(self, x_length, y_length, co_ord_symbol="o "):
		# creates a multi-dimensional array of n * m length
	...
	def no_place_for_old_guess(self, row, col):
		# Replaces the co-ordinate at wrong guessed ship position for convinience with X
...
def battleshipUI():
	...
	while turns < max_turns:
		...
		row_guess = int(input("\nGuess row? ")) - 1
		col_guess = int(input("Guess column? ")) - 1

		# Conditions if user guessed correctly, or missed it or guessed extremely wrong co-ordinates
		if lets_battle.is_ship_there(row_guess, col_guess):
			...
			print("\n - Ship isn't even in Ocean! Your guess is beyond the number of rows and columns!")
		else:
			if lets_battle.get(row_guess, col_guess) == "X ":
